...
class TrainingInterface {
    ...
    updateTrainingSessionsList(trainingSessions) {
        ...
        const sessionsHTML = trainingSessions.map(session => {
            // Define actions for Training tab - use SAME actions as Compare tab
            const actions = [
            ...
            // Use the shared session card component
            if (typeof createSessionCard === 'function') {
                ...
                // Fallback to original rendering if createSessionCard is not available
                return this.renderFallbackSessionCard(session);
            }
        }).join('');
        
        // Use a consistent layout (no Bootstrap grid since session cards are already styled)
        container.innerHTML = sessionsHTML;
    }
    
    renderFallbackSessionCard(session) {
        // Fallback method for when createSessionCard is not available
